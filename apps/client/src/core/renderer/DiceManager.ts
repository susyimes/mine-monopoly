...
export class DiceManager {
   ...
   */
  private _createDiceFaceTexture(value: number | string, baseColor: string, isProphecy: boolean = false): THREE.MeshStandardMaterial {
    ...
    ctx.fillText(String(value), 0, isProphecy ? 40 : 20); // 预言状态稍微下移给标签腾位置
  ...
  private _startArranging(): void {
    ...
    this.diceObjects.forEach((obj, i) => {
      ...
      // --- Prophecy Logic Check ---
      // 在开始展示前，如果存在预言值，强制替换目标面的纹理
      if (obj.prophecyValue !== undefined && obj.prophecyValue !== null) {
          ...
          // 注意：不需要修改 obj.mesh.material 引用，因为它是数组引用
      }
      // ----------------------------
